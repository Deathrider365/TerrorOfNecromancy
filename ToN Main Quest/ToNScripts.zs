    ...
    void run(int msg, int dmap, int scr, int timeUntilWarp)
    {
		...
		while(true)
		{
			...
			if (timer == timeUntilWarp)
			{
				...
				Hero->WarpEx({WT_ENTRANCEEXIT, dmap, scr, -1, WARP_A, WARPFX_WAVE, 0, 
					WARP_FLAG_SETCONTINUESCREEN | WARP_FLAG_SETCONTINUEDMAP, DIR_UP});
...
//~~~~~~AdvanceBoomerangPickUp~~~~~~//
item script AdvanceBoomerangPickUp
{
	void run()
	{
		//Link->Item[23] = false;
...
eweapon script Leviathan_Waterfall
{
	void run(int width, int peakHeight)
	{
		...
		int i;
		int x;
		if(!bitmaps::inuse(leviathan_waterfall_bmp))
		{
			this->DeadState = 0;
			Quit();
		}
		...
		while(waterfallTop>peakHeight)
		{
			...
			for(i=0; i<width; ++i)
			{
				...
				leviathan_waterfall_bmp->Blit(0, -2, 0, 0, 16, bgHeight, x, waterfallTop, 16, bgHeight, 0, 0, 0, BITDX_NORMAL, 0, false);
			}
			Waitframe();
		}
		bgHeight = waterfallBottom-waterfallTop;
		...
		while(waterfallBottom<176)
		{
			...
			{
				x = startX-(width-1)*8+i*16;
				leviathan_waterfall_bmp->Blit(0, -2, 0, 0, 16, bgHeight, x, peakHeight, 16, bgHeight, 0, 0, 0, BITDX_NORMAL, 0, false);
				leviathan_waterfall_bmp->Blit(4, -2, 16, 175-fgHeight, 16, fgHeight, x, peakHeight, 16, fgHeight, 0, 0, 0, BITDX_NORMAL, 0, false);
			}
			Waitframe();
		}
		while(waterfallTop<176)
		{
			...
			{
				x = startX-(width-1)*8+i*16;
				leviathan_waterfall_bmp->Blit(4, -2, 16, 175-fgHeight, 16, fgHeight, x, waterfallTop, 16, fgHeight, 0, 0, 0, BITDX_NORMAL, 0, false);
			}
			Waitframe();
		}
		this->DeadState = 0;
...
bitmap status_bmp;
bitmap leviathan_waterfall_bmp;
...
const int SPR_LEVIATHAN_WATERBALL = 94;
...
npc script Leviathan1
{
	...
	{		
		Hero->Dir = DIR_UP;
		if (leviathan_waterfall_bmp && bitmaps::inuse(leviathan_waterfall_bmp))
			bitmaps::release(leviathan_waterfall_bmp);
			
		leviathan_waterfall_bmp = bitmaps::acquire(32, 176);
		...
		while(true)
		{
			...
			switch(attack)
			{
				case 0:
					...
					for(i = 0; i < 20; ++i)
					{
						Leviathan_GlideFrame(this, vars, x2, 32, x2, 112, 20, i);
						if(i == 3)
						{
							eweapon waterfall = CreateEWeaponAt(EW_SCRIPT10, this->X+80, 112);
							waterfall->Damage = this->Damage;
							waterfall->Script = Game->GetEWeaponScript("Leviathan_Waterfall");
	...
	void Leviathan_UpdateWaterfallBitmap()
	{
		int cmb;
		leviathan_waterfall_bmp->Clear(0);
		int ptr[5*22];
		for(int i=0; i<11; ++i)
		{
			cmb = CMB_WATERFALL;
			if(i==0)
				cmb = CMB_WATERFALL+1;
			leviathan_waterfall_bmp->FastCombo(0, 0, 16*i, cmb, CS_WATERFALL, 128);
			
			cmb = CMB_WATERFALL+2;
			if(i==10)
				cmb = CMB_WATERFALL+3;
			leviathan_waterfall_bmp->FastCombo(0, 16, 16*i, cmb, CS_WATERFALL, 128);
		}
	}
	...
	void Leviathan_Waitframe(npc this, untyped vars)
	{
		...
		Leviathan_UpdateWaterfallBitmap();
